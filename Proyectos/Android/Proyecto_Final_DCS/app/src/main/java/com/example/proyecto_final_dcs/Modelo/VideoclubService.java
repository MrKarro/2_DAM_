...
public class VideoclubService {
    ...
    private Retrofit getRetrofit() {
        return new Retrofit.Builder()
                .baseUrl("http://172.23.86.211/app-dcs/")
    ...
    public Call<Usuario> getUsuario(String login, String password){
        return getRetrofit().create(VideoclubDAO.class).getUsuario(login, password);
    }
    ...
    public Call<Void> eliminarAlquiler(int idPelicula, int idUsuario){
        return getRetrofit().create(VideoclubDAO.class).eliminarAlquiler(idPelicula, idUsuario);
    }

    public Call<Void> eliminarPelicula(int idPelicula){
        return getRetrofit().create(VideoclubDAO.class).eliminarPelicula(idPelicula);
    }
    public Call<Void> cambiaContra(int idUsuario, String nuevaContra){
        return getRetrofit().create(VideoclubDAO.class).cambiaContra(idUsuario, nuevaContra);
    }

    public Call<Void> setExtendido(int idAlquiler, int extendido){
        return getRetrofit().create(VideoclubDAO.class).setExtendido(idAlquiler, extendido);
    }


    public Call<Usuario> getUsuarioId(int idUsuario){
        return getRetrofit().create(VideoclubDAO.class).getUsuarioId(idUsuario);
    }

    public Call<List<Alquiler>> getAlquileresDNI(String dni){
        return getRetrofit().create(VideoclubDAO.class).getAlquilerDNI(dni);
    }
